...
const RoomDetail = () => {
    ...
    const [filteredRoom, setFilteredRoom] = useState([]); // State to store filtered rooms
    const navigate = useNavigate();


    const fetchRoom = async () => {
        try {
            const response = await apiClient.get(`${baseURL}/api/room/getRoom`);
            setRoom(response.data);
             setFilteredRoom(response.data); // Initialize filtered data with all rooms
    ...
    setSearchTerm(searchValue);
      ...
      const filtered = room.filter((room) =>
         room.roomNumber.toString().includes(searchValue) ||
          ...
          room.roomCapacity.toString().includes(searchValue) ||
          room.tariff.toString().includes(searchValue) 
      );
    ...
    const currentRoom = filteredRoom.slice(indexOfFirstProduct, indexOfLastProduct);
    const totalPages = Math.ceil(filteredRoom.length / roomPerPage);

...
const New = () => {
  ...
  const fetchRoomData = async () => {
    try {
      ...
      const convertFeaturesArrayToObject = (featuresArray) => {
        const availableFeatures = ['wifi', 'ac', 'tv', 'breakfast', 'parking'];
      ...
      const populatedRooms = Array.from({ length: roomCount }, (_, i) => {
        if (i < roomData.length) {
          // If room data exists, map the features array to an object
          return {
            ...roomData[i],
            features: convertFeaturesArrayToObject(roomData[i].features),
          };
        }
        // Default empty room if no room data exists
        return {
          roomNumber: "",
          tariff: "",
          roomCapacity: "",
          roomType: "",
          status: true,
          features: { wifi: false, ac: false, tv: false, breakfast: false, parking: false },
        };
      });
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`${baseURL}/api/room/addRooms`, { rooms: data.rooms });
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex space-x-4 mb-4">
                ...
                <div className="w-2/3">
                  ...
                  <div className="space-x-2">
                    <label>
                      <input type="checkbox" {...register(`rooms.${index}.features.wifi`)} defaultChecked={field.features.wifi} /> Wi-Fi
                    </label>
                    <label>
                      <input type="checkbox" {...register(`rooms.${index}.features.ac`)} defaultChecked={field.features.ac} /> AC
                    </label>
                    <label>
                      <input type="checkbox" {...register(`rooms.${index}.features.tv`)} defaultChecked={field.features.tv} /> TV
                    </label>
                    <label>
                      <input type="checkbox" {...register(`rooms.${index}.features.breakfast`)} defaultChecked={field.features.breakfast} /> Breakfast
                    </label>
                    <label>
                      <input type="checkbox" {...register(`rooms.${index}.features.parking`)} defaultChecked={field.features.parking} /> Parking
                    </label>
                  </div>
                </div>
          ...
          <div className="flex justify-center mb-4">
            <button
              ...
              onClick={() => append({ roomNumber: "", tariff: "", roomCapacity: "", roomType: "", status: true, features: { wifi: false, ac: false, tv: false, breakfast: false, parking: false } })}
            >
              Add Room
            </button>
          </div>

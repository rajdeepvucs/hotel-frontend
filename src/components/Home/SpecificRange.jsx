...
//                   {room.roomType} - Tariff - {room.tariff}
//                 </div>
//               </th>
//             ))}
//           </tr>
...
//                       let statusText = "";
//                       if (status === "checkIn") statusText = "Occupied";
...
//                       return (
//                         <span key={index} className={`${bgColor} text-center block w-full p-2`}>
//                           {statusText}
//                           {index < statuses.length - 1 && <br />}{" "}
//                           {/* Add line break if multiple statuses */}
//                         </span>
//                       );
//                     })}
//                   </td>
...
//             {filteredRooms.map((room) => {
//               const isDisabled = bookedRoom.some(
//                 (item) => item.roomName === room.roomNumber.toString()
//               );
//               return (
...
const SpecificRange = ({
  ...
  useEffect(() => {
    ...
    fetchBookings();
    fetchBookedRoom();
  }, [startDate, endDate]); // Added endDate to the dependency array
  ...
  const getBookingStatus = (currentDate, roomNumber) => {
    ...
    for (let booking of bookingList) {
      ...
      if (currentDate >= checkInDate && currentDate < checkOutDate) {
       ...
       // Additional check for same date
     // If currentDate is the same day as checkOutDate, the room is available.
     if (
      currentDate.getFullYear() === checkOutDate.getFullYear() &&
      currentDate.getMonth() === checkOutDate.getMonth() &&
      currentDate.getDate() === checkOutDate.getDate()
    ) {
      return "available";
    }
    }
        ...
        <tbody>
          {filteredDays.map((date) => (
            <tr key={date}>
              ...
              {filteredRooms.map((room) => {
                const status = getBookingStatus(date, room.roomNumber);
                const bgColor =
                ...
                return (
                  <td
                    ...
                    className={`px-2 py-2 border three-d-effect`}
                  >
                    <span className={`${bgColor} text-center block w-full p-2`}>
                      {statusText}
                    </span>
                  </td>
                );

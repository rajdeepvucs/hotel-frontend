...
const TotalRenenueCollection = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await apiClient.get(`${baseURL}/api/getMonthWiseTotalAmount`); // Replace with actual API endpoint
    ...
    const transformedData = data.reduce((acc, item) => {
        ...
        if (yearIndex > -1) {
          acc[yearIndex][item.month] = item.totalAmountReceived;
        } else {
          const newRow = { year: item.year };
          months.forEach((month) => (newRow[month] = "-"));
          newRow[item.month] = item.totalAmountReceived;
          acc.push(newRow);
        }
        return acc;
      }, []);
  ...
  return (
    <div className='flex-1 overflow-auto relative z-10'>
      <Header title={"Total Revenue Collection"}/>
      <motion.div 
...
export default TotalRenenueCollection;
